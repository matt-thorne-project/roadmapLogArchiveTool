...
rotateFunction () {
    ...
    cp /var/log/$folderName/access.$currentBackupDate.log /mnt/backup
    cp /var/log/$folderName/error.$currentBackupDate.log /mnt/backup

    rm /var/log/$folderName/access.$deleteDate-*.log.gz
    rm /var/log/$folderName/error.$deleteDate-*.log.gz

    echo "$currentDate - Backup completed" >> /backup/logRotate.log;
}


#Check if a folder name has been passed into the arg then rotate logs if it has been supplied
if [ -z "$1" ]
    then echo "$currentDate - No file path entered" >> /backup/logRotate.log;
    else rotateFunction;
fi
